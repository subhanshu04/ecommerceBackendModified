...
public class Authentication {
    ...
    public ResponseEntity login(@RequestBody LoginRequestBody loginRequestBody){
        ...
        // If user is not verified then we need to catch the exception and print the Reason for failure.
    ...
    // Spring on getting authentication cast the authentication to User and provide the user with authentication.
    @GetMapping("/me")
    public User getLoggedInUserProfile(@AuthenticationPrincipal User user){
        //@AuthenticationPrincipal will get you the user which is associated to the Token provided in header. This uses getUsername method in JWTService Class.
        return user;
    }
    ...
    public ResponseEntity requestPasswordResetLink(@RequestBody LoginRequestBody loginRequestBody) throws CouldNotSentEmailException, UserNotFoundException, WrongPasswordException {
        String message =  userService.requestPasswordResetLink(loginRequestBody);
        return ResponseEntity.ok().body(message);
    }

    @PostMapping("/resetPassword")
    public ResponseEntity resetPassword(@RequestParam String token,@RequestParam String password) throws UserNotFoundException {
